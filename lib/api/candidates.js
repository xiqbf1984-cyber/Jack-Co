...
export async function fetchCandidates(orgId) {
  ...
  // Count assessments per candidate (candidates with a challenge_id)
  var candidates = data.map(toAppCandidate);
  var assessmentCounts = {};
  data.forEach(function (row) {
    if (row.challenge_id) {
      assessmentCounts[row.id] = (assessmentCounts[row.id] || 0) + 1;
    }
  });
  candidates.forEach(function (c) {
    if (assessmentCounts[c.id]) c.assessments = assessmentCounts[c.id];
  });
  return candidates;
}
