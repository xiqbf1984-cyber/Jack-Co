...
// Main fetch: org-linked assessments + any created by this host (so 0-candidate
// assessments are still visible to their creator)
export async function fetchAssessments(hostId, orgId) {
  var orgAssessments = await fetchAssessmentsByOrg(orgId) || [];
  var hostAssessments = await fetchAssessmentsByHost(hostId) || [];

  // Merge, deduplicating by id (org results take priority)
  var seen = new Set(orgAssessments.map(function (a) { return a.id; }));
  var extra = hostAssessments.filter(function (a) { return !seen.has(a.id); });
  return orgAssessments.concat(extra);
}
