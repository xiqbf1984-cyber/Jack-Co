...
function toAppRole(row) {
  return {
    ...
    hiringProfile: row.hiring_profile || null,
    sharableLink: row.sharable_link || null,
  };
}
...
export async function createRole(orgId, role, createdBy) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      chat_history: role.chatHistory || null,
      hiring_profile: role.hiringProfile || null,
      sharable_link: role.sharableLink || null,
    })
    .select()
    .single();
...
export async function updateRoleDb(roleId, updates) {
  ...
  if (updates.hiringProfile !== undefined) dbUpdates.hiring_profile = updates.hiringProfile;
  if (updates.sharableLink !== undefined) dbUpdates.sharable_link = updates.sharableLink;
