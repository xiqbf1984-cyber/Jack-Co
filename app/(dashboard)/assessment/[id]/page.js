      ...
      {activeTab === 'candidates' && (
        <div className="space-y-3 animate-fsu">
          {(challenge.results || []).map((result) => {
            ...
            return (
              <div key={result.candId} className="rounded-xl border flex items-center gap-4" style={{ padding: 'var(--row-padding-y) var(--row-padding-x)', backgroundColor: 'var(--cream-card)', borderColor: 'var(--border-default)', boxShadow: 'var(--shadow-card)' }}>
